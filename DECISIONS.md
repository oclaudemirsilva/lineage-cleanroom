...
## DEC-7 — `.h5ad` adapter: container-correct, domain-agnostic, optional; PPI deferred
**Context.** The named user runs single-cell (Perturb-seq / CRISPR) screens, whose standard container is
**AnnData `.h5ad`**. An adapter for it is the highest-value "ready for *your* data on day one" artifact.
**Decision.** Ship a `.h5ad` adapter that maps AnnData → the existing `SplitView` contract and *nothing
more*: `.X` (or a named `.obsm` embedding) → features; `.obs` columns → label / group / provenance /
split. The column **roles stay caller-supplied** (CLI args), exactly like the CSV adapter — so the tool
stays domain-agnostic and does **not** guess which column is the donor/label. `anndata` is an
**optional, lazily-imported** dependency; the core remains numpy/sklearn/cryptography-only and its tests
pass without it. Validated with a **positive *and* negative control** (a donor spanning the split is
caught; a group-aware split is *not* flagged) and end-to-end through the CLI on real `.h5ad` files.
**Deferred: PPI edge-lists.** Graph (protein–protein interaction) leakage is a *different* problem —
node/edge overlap across graph splits, not row duplication or group-spanning — so a PPI adapter would
need its own detector, not just a loader. Shipping a loader without the matching detector would overclaim;
deferred until the detection semantics are built. **Delegation:** the adapter body was generated by the
OpenRouter "super squad" (DeepSeek) under a written spec and a human/Claude review gate — per DEC-1.
